...
var paramsNewAddress = {
    ...
    //Caso seja um Market Place, remova os coment�rios abaixo alterando o valor do MarketPlaceToken para cada parte envolvida.
    //split: [
...
WarpExchange.getnewaddress(paramsNewAddress, function(err, response) {
    if (!err) {
        ...
        if (response.statusCode === 200) {
            ...
            console.log(
                'Valor em BTC: ' +
                    WarpExchange.ToBTC(response.body.valueInSatoshi),
            )
        } else {
            console.log(response)
        }
...
var paramTID = {
    merchantSystemID: 'YourSystemID', //O identificador do seu sistema, enviado como par�metro quando solicitado no m�todo "getnewaddress".
}
